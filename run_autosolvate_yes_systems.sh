...
BOXGEN_JSON="/home/xuchen/rfmm/scripts/boxgen.json"
...
# Make sure the boxgen.json file exists
if [ ! -f "$BOXGEN_JSON" ]; then
    echo "Error: boxgen.json file $BOXGEN_JSON does not exist!"
    exit 1
fi
...
for folder in "${YES_LABELED_SYSTEMS[@]}"; do
    ...
    # Change to the folder and run the autosolvate command
    (
        cd "$folder_path" || { 
        ...
        echo "  Running autosolvate boxgen_multicomponent in $folder..."
        if autosolvate boxgen_multicomponent -f "$BOXGEN_JSON"; then
            echo "  Successfully ran autosolvate in $folder"
        else
            echo "  Failed to run autosolvate in $folder"
            failed=$((failed+1))
        fi
    )
done
